...
def calculate_summary(selections, guest_count, menu_data, upsells):
    ...
    return {"breakdown": breakdown, "total_dishes": total_dishes, "estimated_cost": estimated_cost}
...
def export_to_csv(selections, dietary_notes, guest_count, couple_name, menu_data, upsells):
    ...
    for category, dishes in selections.items():
        ...
        for dish_name in dishes:
            ...
            rows.append({
                "Para": couple_name, "Kategoria": category, "Danie": dish_name,
                "Porcje": guest_count,
    ...
    for upsell_name in selections.get("_upsells", []):
        upsell = next((u for u in upsells if u["name"] == upsell_name), None)
        rows.append({
            "Para": couple_name, "Kategoria": "Dodatek Premium", "Danie": upsell_name,
            "Porcje": "-", "Cena/os (zł)": "-",
            "Suma (zł)": upsell.get("price", "-") if upsell else "-", "Alergeny": "",
        })
    rows.append({
        "Para": couple_name, "Kategoria": "DIETY",
        "Danie": (f"Wegetarianie: {dietary_notes.get('vegetarian',0)}, "
                  f"Weganie: {dietary_notes.get('vegan',0)}, "
                  f"Bezglutenowi: {dietary_notes.get('gluten_free',0)}"),
        "Porcje": "", "Cena/os (zł)": "", "Suma (zł)": "",
